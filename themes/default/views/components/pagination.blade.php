        ...
        <nav role="navigation" aria-label="Pagination Navigation" class="flex gap-2 items-center">
            ...
            <span>
                @if ($paginator->onLastPage())
                    <span class="bg-primary-700 text-white px-4 py-2 rounded-lg">{{ __('pagination.next') }}</span>
                @else
                    <button wire:click="nextPage" wire:loading.attr="disabled" rel="next"
                        class="bg-primary-700 text-white px-4 py-2 rounded-lg">{{ __('pagination.next') }}</button>
                @endif
            </span>
        </nav>
    @endif
