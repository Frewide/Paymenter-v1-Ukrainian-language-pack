<div @if ($checkPayment) wire:poll="checkPaymentStatus" @endif>
    ...
    <div class="bg-primary-800 p-12 rounded-lg mt-2">
        <div class="sm:flex justify-between pr-4 pt-4">
            <h1 class="text-2xl font-bold sm:text-3xl">{{ __('invoices.invoice_no') }} #{{ $invoice->id }}</h1>
        ...
        <div class="sm:flex justify-between pr-4 pt-4">
            <div class="mt-6">
                <p class="uppercase font-bold">{{ __('invoices.invoice_from') }}</p>
            ...
            <div class="max-w-[200px] w-full mt-6">
                ...
                @elseif ($invoice->status == 'pending')
                        ...
                        @if ($checkPayment)
                            <div class="mt-4">
                                <x-button.primary wire:click="checkPaymentStatus" wire:loading.attr="disabled"
                                    ...
                                    <span class="sr-only">{{ __('invoices.invoice_loading') }}</span>
                    ...
                    <x-button.primary wire:click="pay" class="mt-4" wire:loading.attr="disabled" wire:target="pay">
                        <span wire:loading wire:target="pay">{{ __('invoices.invoice_processing') }}</span>
                        <span wire:loading.remove wire:target="pay">{{ __('invoices.invoice_pay') }}</span>
                    </x-button.primary>
                @endif
            </div>
        </div>
        ...
        @if ($invoice->transactions->isNotEmpty())
            <div class="mt-12">
                <h2 class="text-2xl font-bold">{{ __('invoices.transcations') }}</h2>
